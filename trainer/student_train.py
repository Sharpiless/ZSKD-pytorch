...
class s_trainer():
    def __init__(self, dataset, data_path, teacher, student, save_path, test_path='./trainer/dataset/'):
        ...
        self.data_train = torchvision.datasets.ImageFolder(
            root=self.data_path, transform=train_trans)
        ...
        if self.dataset == 'mnist':
            ...
            self.optimizer = torch.optim.Adam(
                self.student.parameters(), lr=0.001, weight_decay=1e-4)
            ...
            self.optimizer = torch.optim.SGD(
                self.student.parameters(), lr=0.1, momentum=0.9, weight_decay=1e-4)

        self.trainset_loader = DataLoader(
            self.data_train, batch_size=batch_size, shuffle=True, num_workers=0)
    ...
    def train(self, epoch):
        ...
        for i, (images, labels) in enumerate(self.trainset_loader):
            ...
            loss = -torch.sum(F.log_softmax(outputs_S/20, dim=1)
                              * F.softmax(outputs_T/20, dim=1).detach()) / images.size(0)
    ...
    def test(self):
        ...
        print('Test Avg. Loss: %f, Accuracy: %f, Best Accuracy: %f' %
              (avg_loss.data.item(), self.acc, self.acc_best))
